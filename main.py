...
class Server:
  ...
  def start(self):
    ...
    if not self.status():
      mcServer = pexpect.spawn('screen -S mc ' + self.startupScript)
      mcServer.expect('\[INFO\] Done')
      print 'Started'
      return mcServer
    else:
  ...
  def command(self, command, sleep=0.07):
    if self.status():
  ...
  #only return that many lines still starting from n lines back
  ...
  #Return the last n lines of chat history
  def chat(self, n=10):
    searchBack = 50
    chatLines = []
    while len(chatLines) < n:
      logLines = self.console(searchBack,50)
      for line in logLines:
        match = re.search(r'<\w+>', line)
        match2 = re.search(r'\[CONSOLE\]', line)
        if match or match2:
          chatLines.append(line)
      searchBack = searchBack + 50
    return sorted(chatLines)
...
def main():
  ...
  if not, the instantiated server will boot up a new server.
